...
pub fn expand_with<E>(env: &mut E, input: &str) -> String
where
    E: Enviroment,
{
    parser::parse(env)(input.as_bytes()).unwrap().1.unwrap()
}
...
mod tests {
    ...
    fn test_constant() {
        ...
        assert_eq!(expand("$").as_str(), "$");
        assert_eq!(expand("bar$").as_str(), "bar$");
    }

    #[test]
    fn test_empty() {
        assert_eq!(expand("").as_str(), "");
        assert_eq!(expand("$foo").as_str(), "");
    }
}
